...
def resize_image_for_retention(
    ...
    """Resize image area by a target visual-token retention ratio.

    Qwen2.5-VL's post-merge visual-token grid is roughly one token per
    28x28 pixels. Scaling image width and height by sqrt(retention_ratio)
    approximates an early token budget before processor and vision encoder
    computation. This is a baseline, not the same algorithm as pruning or
    token merging after visual embeddings are computed.
    """
...
def run_early_resize_vqa_quality_sweep(
    model,
    processor,
    samples: list[dict[str, Any]],
    schema_report,
    ...
    """Evaluate VQA quality for the early resize token-budget baseline."""
    retention_ratios = retention_ratios or [1.0, 0.75, 0.5, 0.25, 0.1]
    question_field = schema_report.question_field or "question"
    image_field = schema_report.image_field or "image"
    metric_name = schema_report.selected_metric
    ...
    for ratio in retention_ratios:
        ...
        for sample_idx, sample in enumerate(samples):
            question = sample[question_field]
            image = resize_image_for_retention(sample[image_field], ratio)
            prompt = prompt_template.format(question=question)
            inputs = build_qwen_inputs(processor, device, image, prompt)
...
def _time_early_resize_total(
    ...
    def timed(name: str, fn: Callable[[], Any]):
        ...
        if torch.cuda.is_available():
            torch.cuda.synchronize()
        stage_times[name] = (time.perf_counter() - start) * 1000
        return result

    resized = timed("resize_ms", lambda: resize_image_for_retention(image, ratio))
    inputs = timed("processor_total_ms", lambda: build_qwen_inputs(processor, device, resized, prompt))
    image_token_id = get_image_token_id(model, processor)
    debug = {
        "resized_width": resized.size[0],
        "resized_height": resized.size[1],
        "visual_tokens_after": count_visual_tokens(inputs, image_token_id),
    ...
    timed("generate_ms", generate)
    total_ms = sum(stage_times.values())
    return total_ms, stage_times, debug


def run_early_resize_performance_sweep(
    model,
    processor,
    device: str = "cuda",
    ...
    """Measure the formal early resize baseline with end-to-end timing.

    The reported ``latency_ms`` includes resizing, processor/tokenization,
    device transfer through ``build_qwen_inputs``, and generation. This makes
    the result conservative for early compression because it includes its setup
    overhead rather than only timing model.forward/generate.
    """
    retention_ratios = retention_ratios or [1.0, 0.75, 0.5, 0.25, 0.1]
    ...
    for ratio in retention_ratios:
        ...
        for sample_idx in range(num_samples_per_cell):
            ...
            for run_idx in range(num_runs):
                ...
                if torch.cuda.is_available():
                    torch.cuda.reset_peak_memory_stats()
                total_ms, stage_times, debug = _time_early_resize_total(
                    model,
                    processor,
                    device,
                    image,
        ...
        peak_memory = (
            float(run_df["peak_memory_mb"].dropna().max())
            if not run_df.empty and run_df["peak_memory_mb"].notna().any()
            else None
        )
        rows.append(
            {
                ...
                "resized_width": float(run_df["resized_width"].mean()) if not run_df.empty else None,
                "resized_height": float(run_df["resized_height"].mean()) if not run_df.empty else None,
                "visual_tokens_full": None,
                "visual_tokens_after": float(run_df["visual_tokens_after"].mean()) if not run_df.empty else None,
                "input_sequence_length": float(run_df["input_sequence_length"].mean()) if not run_df.empty else None,
                "latency_ms": float(run_df["latency_ms"].mean()) if not run_df.empty else None,
                "latency_std_ms": float(run_df["latency_ms"].std()) if len(run_df) > 1 else 0.0,
                "resize_ms": float(run_df["resize_ms"].mean()) if "resize_ms" in run_df else None,
                "processor_total_ms": float(run_df["processor_total_ms"].mean()) if "processor_total_ms" in run_df else None,
                "generate_ms": float(run_df["generate_ms"].mean()) if "generate_ms" in run_df else None,
                "peak_memory_mb": peak_memory,
                "n_samples": num_samples_per_cell,
                "n_runs": num_runs,
                "failures": "",
            }
        )
    ...
    baseline_tokens = df[df["retention_ratio"] == 1.0]["visual_tokens_after"].mean()
    if baseline:
        df["speed_vs_full_resolution"] = baseline / df["latency_ms"]
    if baseline_tokens:
        df["effective_token_retention"] = df["visual_tokens_after"] / baseline_tokens
        df["visual_tokens_full"] = baseline_tokens
    return df
